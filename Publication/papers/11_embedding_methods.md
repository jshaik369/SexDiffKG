...
Knowledge graph embedding (KGE) models have emerged as a foundational tool for representation learning over structured relational data, with applications in drug repurposing [1], adverse drug reaction prediction [2], target identification [3], and clinical trial design [4]. KGE models learn continuous, low-dimensional vector representations of entities (drugs, genes, diseases) and relations (targets, causes, treats), enabling link prediction and similarity computation without explicit feature engineering. The central insight is that the geometric structure of the embedding space can reflect the relational semantics of the knowledge graph.

The KGE literature traces its origins to translational models. **TransE** [5] (Bordes et al., 2013) models each relation as a translation vector: for a true triple (h, r, t), **h** + **r** approximates **t**. TransE is efficient but cannot model symmetric relations (if **h** + **r** = **t**, then **t** + **r** cannot equal **h** unless **r** = 0) or one-to-many relations, motivating increasingly expressive models.

**DistMult** [6], proposed by Yang et al. in 2015, replaced the translational scoring function with a bilinear diagonal model. Each relation is a diagonal matrix, and the score for a triple (h, r, t) is the trilinear dot product: sum of h_i * r_i * t_i. DistMult is computationally efficient and competitive despite a fundamental limitation: the scoring function is symmetric in h and t (f(h,r,t) = f(t,r,h)), meaning it cannot distinguish asymmetric relations. In practice, DistMult partially compensates through training dynamics, but the constraint remains a ceiling on expressiveness.

**ComplEx** [7], introduced by Trouillon et al. in 2016, extended DistMult to complex-valued vector spaces. By representing entities and relations as complex vectors, ComplEx achieves asymmetric scoring through the Hermitian dot product: Re(sum of h_i * r_i * conj(t_i)), where the conjugation of the tail embedding breaks h/t symmetry. ComplEx can model symmetric, antisymmetric, and inverse relation patterns simultaneously---crucial for heterogeneous biomedical KGs where different relations exhibit different symmetry properties.

**RotatE** [8], proposed by Sun et al. in 2019, models each relation as an element-wise rotation in complex space. Each relation embedding has unit modulus (|r_i| = 1), and the scoring function measures ||h_i * r_i - t_i||. RotatE can model symmetric, antisymmetric, inverse, and---uniquely---composition patterns (relation A followed by B equals C), corresponding to element-wise rotation multiplication.

Beyond these, **ConvE** [9] introduced CNNs over reshaped embeddings, and **TuckER** [10] formulated KGE as Tucker decomposition, subsuming several earlier models as special cases.
...
In the pharmacovigilance domain specifically, KG approaches have gained traction because FAERS and similar spontaneous reporting systems generate structured relational data (drugs, adverse events, demographics) that naturally form graph structures, and integrating these signals with molecular data enables mechanistic interpretation. Celebi et al. [2] found that tensor factorization methods (including ComplEx) outperformed translational models for drug-drug interaction prediction. Mohamed et al. [1] demonstrated KGE-based protein target discovery. Chandak et al. [4, 16] built PrimeKG (129,375 nodes, 8,100,498 edges) for disease-specific drug prioritization---though without sex-differential analysis.

Despite this progress, critical gaps remain. First, most evaluations focus on benchmark datasets (FB15k-237, WN18RR) rather than domain-specific pharmacovigilance graphs with distinct structural properties. Second, the impact of graph scale on embedding quality is rarely characterized systematically. Third, the relationship between embedding similarity and sex-differential safety profiles is unexplored. Fourth, the pharmacovigilance KG literature has largely ignored sex as a biological variable, despite evidence that adverse drug reactions exhibit significant sex-differential patterns [14].
...
SexDiffKG was constructed through a multi-source integration pipeline designed to capture drug safety signals at both the population and molecular levels. The graph schema follows a hub-and-spoke design centered on Drug entities, connecting outward to AdverseEvent nodes (population-level safety), Gene/Protein nodes (molecular mechanism), Pathway nodes (biological context), and Tissue nodes (expression context). This design supports mechanistic reasoning: given a drug with a sex-differential adverse event signal, the graph enables traversal through drug targets, interacting proteins, pathway memberships, and tissue expression patterns to identify candidate molecular mechanisms.

Gene and Protein entities are maintained as separate types because the mapping is not one-to-one: alternative splicing and protein complexes mean a single gene can correspond to multiple functional protein entities. The 77,498 Gene entities correspond to HGNC identifiers, while the 16,201 Protein entities correspond to UniProt accession numbers from ChEMBL target annotations.

The six v4 relation types capture distinct biological semantics: **has_adverse_event** (Drug -> AdverseEvent, 869,142 edges); **sex_differential_adverse_event** (Drug -> AdverseEvent, 96,281 edges, derived from sex-stratified Reporting Odds Ratios with Bonferroni-corrected chi-squared test); **interacts_with** (Protein <-> Protein, 473,860 edges from STRING v12.0, score >= 700, the only symmetric relation); **participates_in** (Gene -> Pathway, 370,597 edges from Reactome); **targets** (Drug -> Protein, 12,682 edges from ChEMBL 36, pChEMBL >= 6.0); and **sex_differential_expression** (Gene -> Tissue, 289 edges from GTEx v8).

This distribution is highly skewed: has_adverse_event accounts for 56.7% of edges, while sex_differential_expression accounts for only 0.019%. This skewness has direct implications for KGE training, as models must learn representations for both abundant and rare relation types.
...
**Hyperparameter selection rationale.** The embedding dimension of 200 balances expressiveness with computational cost, within the range recommended by Ruffinelli et al. [17] for medium-scale KGs. The learning rate (0.001, Adam) follows standard KGE practice [15]. Negative sampling with 64 corrupted triples per positive was selected based on preliminary experiments showing diminishing returns beyond 64 negatives for our graph density (~14 edges/node). The 90/5/5 split reserves ~153,000 unique triples for evaluation.

**Negative sampling strategy.** We employed uniform negative sampling: for each positive triple (h, r, t), 64 corrupted triples are generated by replacing head or tail with a random entity. Uniform sampling does not account for entity type constraints (e.g., replacing Drug with Gene in a targets triple produces trivially false negatives). Type-constrained [18] or self-adversarial [8] negative sampling may improve performance but were not explored due to CPU training cost. Inverse triples (adding (t, r_inv, h) for each (h, r, t)) effectively double training data and improve bilinear model performance [15].
...
The filtered setting [5] removes all known true triples (from train, validation, and test sets) except the triple being evaluated, preventing penalization for correctly predicting known-positive triples. This is critical for dense pharmacovigilance graphs with many known drug-AE pairs. Both head prediction (given r, t, predict h) and tail prediction (given h, r, predict t) are performed; metrics are averaged across both directions.
...
**Why ComplEx outperforms DistMult: relation symmetry analysis.** The 145% MRR gap warrants deeper analysis. DistMult's scoring function f(h, r, t) = sum(h_i * r_i * t_i) is algebraically symmetric: f(h, r, t) = f(t, r, h). In SexDiffKG v4, four of six relation types are asymmetric (has_adverse_event, sex_differential_adverse_event, targets, participates_in), accounting for 87.7% of all edges. Only interacts_with (30.9%) is symmetric. ComplEx resolves this through the Hermitian product where tail conjugation breaks symmetry. RotatE also models asymmetry through rotation angles, but its distance-based scoring is less discriminative in SexDiffKG's high entity-to-relation ratio setting (22,600:1).

**Comparison to published biomedical KG benchmarks.** On DRKG (97K entities, 5.9M triples, 107 relations), ComplEx achieved MRR ~0.30 [11], 21% higher than our 0.2484---plausible given DRKG's 17x more relation types. On PharmKG (39K entities, 500K triples), DistMult achieves MRR ~0.12 [13], comparable to our 0.1013 on a 2.8x larger graph. On Hetionet-derived graphs, ComplEx MRR ranges from 0.15 to 0.35 [12]. Our results validate SexDiffKG as a challenging but tractable KGE benchmark.
...
The ComplEx advantage can be understood through expressivity theory. Trouillon et al. [7] proved that ComplEx is fully expressive: given sufficient dimensions, it can represent any set of true and false triples exactly. This arises from the complex-valued Hermitian product, which provides 2x the effective parameters per embedding dimension compared to real-valued DistMult. For SexDiffKG v4 with 200 embedding dimensions, ComplEx operates in a 400-dimensional real parameter space per entity versus DistMult's 200 dimensions---a 2x capacity advantage that manifests as the observed 145% MRR gap.

ComplEx's ability to model antisymmetric relations through phase differences is particularly well-suited to pharmacovigilance semantics. The relation has_adverse_event (Drug -> AdverseEvent) encodes causal direction. In ComplEx's Hermitian product, this directionality is encoded through asymmetric conjugation: swapping head and tail produces different scores because conj(h) * t differs from conj(t) * h. DistMult must approximate directionality through entity-specific magnitude biases---an indirect strategy.

RotatE occupies an intermediate position: it models asymmetric relations through non-trivial rotation angles, but its distance-based scoring (||h * r - t||) is less discriminative than ComplEx's inner-product scoring when a single relation type connects thousands of entity pairs (has_adverse_event: 869,142 edges across 3,920 drugs and 9,949 AEs).

### Embedding SexDiffKG in the KG4Drug Landscape

Our results contribute to the growing KG4Drug (Knowledge Graphs for Drug Discovery) ecosystem. Within this landscape, SexDiffKG occupies a distinctive niche as the first knowledge graph explicitly designed for sex-differential pharmacovigilance. Existing drug safety KGs, including Bio-SODA [19], ADEpedia-on-OHDSI [20], and WHO VigiBase-derived graphs, focus on general adverse event detection without sex stratification. SexDiffKG's dedicated relation types (sex_differential_adverse_event, sex_differential_expression) enable queries no existing KG supports, such as identifying drugs with female-predominant hepatotoxicity signals and their distinguishing molecular pathways.

The practical relevance extends to regulatory science. The FDA's 2014 Action Plan on Sex Differences recognized that adverse reactions differ by sex in prevalence, severity, and mechanism, and the EMA has mandated sex-disaggregated clinical trial reporting. KGE-based analysis could complement these efforts by identifying drugs warranting sex-specific label updates.

### Link Prediction for Drug Safety Signal Detection

A key potential application of KGE models on pharmacovigilance KGs is prospective detection of drug safety signals through link prediction. In principle, a trained model could predict the triple (Drug_X, sex_differential_adverse_event, AE_Y) before this signal emerges from FAERS data.

ComplEx's Hits@10 of 0.4069 indicates that the correct adverse event appears in the top 10 predictions for ~41% of test triples---potentially useful for generating ranked watchlists for human review. However, the MRR of 0.2484 (average rank ~4th) and Hits@1 of 0.1678 (correct answer is the top prediction only 16.8% of the time) indicate insufficient precision for autonomous detection. The gap between Hits@1 and Hits@10 (16.8% vs. 40.7%) suggests the model captures neighborhood structure rather than precise entity-level predictions.

This suggests KGE-based link prediction is best deployed as a signal prioritization tool (ranking candidate AEs for expert review) rather than an autonomous detection system. Domain-specific extraction further supports this: REPRODUCT-KG's focused embeddings would provide better signal prioritization for reproductive drug safety than the parent graph's diluted embeddings.
...
All KGE models evaluated here are transductive: they learn fixed embeddings for entities observed during training and cannot generalize to unseen entities. This has key implications for pharmacovigilance: (1) newly approved drugs cannot be embedded without full retraining---precisely the entities for which predictive signals would be most valuable; (2) FAERS receives ~2 million new reports annually, requiring periodic retraining (2--10 hours per model); and (3) embeddings cannot transfer across KGs (e.g., from SexDiffKG to an EMA EudraVigilance-derived graph) due to differing entity identifiers.

Inductive KGE methods such as NodePiece [21], GraIL [22], and NBFNet [23] offer a potential solution by learning entity representations from local graph structure, enabling zero-shot link prediction for new drugs. However, these methods typically require richer local structure than SexDiffKG's hub-and-spoke design provides, and their computational cost is substantially higher.

### Future Directions

Several promising research directions emerge from this work:

**Graph Neural Network (GNN) approaches.** GNN-based methods such as R-GCN [24], CompGCN [25], and SE-GNN [26] propagate information through graph structure, allowing each entity's embedding to be informed by its multi-hop neighborhood. For SexDiffKG, this is relevant because mechanistic interpretation involves multi-hop reasoning: Drug -> targets -> Protein -> interacts_with -> Protein -> participates_in -> Pathway. GNNs could capture these patterns and improve interpretability.

**Temporal knowledge graph embeddings.** FAERS data is inherently temporal, and sex-differential patterns may evolve as prescribing practices change. Temporal KGE methods such as TTransE [27], HyTE [28], and TNTComplEx [29] incorporate time as an additional dimension. Applying temporal KGE to SexDiffKG could reveal how sex-differential signals emerge or attenuate over the 21-year FAERS period (2004--2025).
...
7. **Single random seed**: All experiments used seed 42. Variance across random initializations was not quantified; bootstrap confidence intervals would strengthen claims but require prohibitive multiple training runs under CPU constraints.

8. **Evaluation on held-out triples only**: Filtered evaluation measures recovery of randomly held-out triples, which may not reflect real-world utility where the goal is predicting triples not yet in any knowledge base. Prospective evaluation against newly reported FAERS signals would be more clinically relevant.

---

## Conclusion

We present the first systematic evaluation of knowledge graph embedding models for sex-differential pharmacovigilance, comparing three models across three graph scales and five therapeutic domains. Our key findings---ComplEx superiority, predictable scaling degradation, and domain-specific extraction as a recovery strategy---provide practical guidance for the growing community of researchers applying KGE methods to drug safety. The disconnect between embedding similarity and sex-differential safety profiles highlights the need for domain-aware KGE architectures that explicitly model sex as a modifier of drug safety. These principles---controlled model comparison, systematic scaling analysis, and domain-specific signal-to-noise optimization---will be essential for translating KGE advances into actionable drug safety intelligence.
