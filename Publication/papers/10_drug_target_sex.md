...
The biological foundation lies in the sexually dimorphic transcriptome. The GTEx Consortium [7] reported that approximately 37% of genes show sex-differential expression in at least one tissue, with the most pronounced differences in reproductive tissues, adipose tissue, skeletal muscle, and liver. Lopes-Ramos et al. [8] demonstrated that sex differences in gene regulatory networks are pervasive across human tissues. Their analysis revealed sex-differential targeting of transcription factors leads to tissue-specific pathway rewiring, suggesting the same drug target may function in fundamentally different molecular contexts in males versus females.
...
A positive logR indicates female-biased disproportionality; negative indicates male-biased. Sex-differential signals were defined as drug--AE pairs with $|logR| \geq 0.5$ and $\geq 10$ reports per sex, corresponding to an approximate 1.65-fold sex difference. Total qualifying signals: 96,281, spanning 2,178 drugs and 5,069 AEs.
...
**Protein-protein interaction data.** STRING v12.0 [15] mapped protein-protein interactions among drug targets (combined confidence score >= 0.7), enabling assessment of whether sex-differential safety patterns propagate through target interaction neighborhoods.

**Sex-differential gene expression data.** GTEx v8 [7] provided tissue-specific sex-differential expression across 54 human tissues, enabling assessment of whether targets at the axis extremes show correspondingly extreme sex-differential expression.
...
Enrichment of protein families at the axis extremes was assessed using a permutation test. For each family (kinases, GPCRs, nuclear receptors, ion channels, proteases), the observed mean female fraction was compared to a null distribution from 10,000 random permutations. P-values were Bonferroni-corrected across 5 families.
...
Among the top 100 drugs by signal count, we computed pairwise divergence (absolute difference in mean female fraction) for drug pairs sharing >= 20 adverse events, controlling for AE composition.
...
**Nuclear receptors** are enriched at both extremes: estrogen receptors anchor the female extreme (90.5%F), androgen receptors anchor the male extreme (31.4%F), and mineralocorticoid/glucocorticoid receptors occupy intermediate positions (58--63%F). This bimodal distribution is unique among protein families and reflects their role as molecular mediators of sex-hormone effects on drug pharmacology.

**Kinases** cluster in the neutral zone (56.1%F for multi-target TKIs), but individual kinases vary. EGFR-targeting drugs (erlotinib, gefitinib, osimertinib) average 52.3%F; VEGFR-targeting drugs (sunitinib, sorafenib, pazopanib) average 48.7%F, trending male-biased consistent with the male predominance of renal cell carcinoma; BCR-ABL inhibitors average 54.1%F. Within-family differences reveal that indication epidemiology modulates the target-level sex signal, but kinases as a family remain near the neutral midpoint.

**GPCRs** span a broad range (54--81%F), reflecting their diversity. Serotonin receptors average 57.9%F, opioid receptors 54.6%F, histamine H1 receptors 61.2%F, and CGRP receptors 81.4%F (the outlier, reflecting migraine's female predominance). This breadth reflects GPCR engagement with both immune and neurological pathways.

**Ion channels** show moderate female bias. Voltage-gated sodium channels (carbamazepine, lamotrigine, phenytoin) average 55.8%F; calcium channels (amlodipine, nifedipine, verapamil) average 58.3%F. Potassium channels show the widest within-family variation, reflecting their pleiotropic functions across cardiovascular and dermatological contexts.
...
The DOACs (rivaroxaban, apixaban, dabigatran) converge within 4.3 pp despite targeting different coagulation factors (factor Xa vs. thrombin), suggesting consistent sex-differential pharmacology across the coagulation cascade, consistent with known sex differences in factor VIII, von Willebrand factor, and antithrombin III levels [18].

**Broad signatures (range > 15 pp):** NSAIDs (18.0 pp) and Anti-TNFs (15.2 pp) show substantial within-class variation. For NSAIDs, this reflects diversity of COX-1/COX-2 selectivity ratios: COX-2-selective NSAIDs (celecoxib) trend more female-biased than non-selective NSAIDs (ibuprofen, naproxen), potentially reflecting sex differences in estrogen-induced COX-2 expression [19]. For Anti-TNFs, the variation reflects differences in molecular format (monoclonal antibody vs. PEGylated Fab fragment) and Fc-receptor-mediated effector functions, which contribute independently to sex-differential safety.

**Intermediate signatures (range 5--15 pp):** SSRIs (10.8 pp) and ICIs (13.9 pp) show moderate within-class variation. For SSRIs, variation reflects differences in transporter selectivity and secondary receptor binding (paroxetine with anticholinergic activity vs. highly selective escitalopram). For ICIs, variation reflects differences between PD-1 (nivolumab, pembrolizumab), PD-L1 (atezolizumab), and CTLA-4 (ipilimumab) antibodies.
...
For minoxidil, cardiovascular AEs (hypotension, tachycardia) are strongly male-biased, consistent with greater male hemodynamic sensitivity to vasodilators, while dermatologic AEs (hypertrichosis, skin irritation) are strongly female-biased, reflecting sex-differential KATP channel expression in hair follicle keratinocytes.

Adalimumab (318 signals, 5--98%F) shows a similarly instructive pattern: immune-mediated AEs (infections, tuberculosis reactivation) trend female-biased, while injection site reactions trend less strongly female and metabolic AEs show near-balanced ratios. This within-drug heterogeneity demonstrates that the molecular sex axis operates at the AE level, not just the drug level.
...
The tofacitinib--leuprorelin pair (47.5 pp divergence, 71 shared AEs) is equally instructive. Tofacitinib inhibits JAK1/JAK3, central to cytokine signaling, and is used predominantly for autoimmune conditions (rheumatoid arthritis: 3:1 female predominance). Leuprorelin is a GnRH agonist used for prostate cancer, creating a complex confounding-by-indication scenario. The 71 shared AEs provide substantial power to detect the target-mediated sex difference.
...
The 6:1 asymmetry persists after sex-stratified disproportionality correction and represents a genuine asymmetry in sex-differential drug susceptibility, consistent with the observation that the female immune system creates a broader surface area of drug-susceptible pathways [4].
...
This framework can be formalized as a target-based sex-safety score:

$$\hat{F}_{drug} = \frac{\sum_{i=1}^{k} w_i \cdot \bar{F}_{T(t_i)}}{\sum_{i=1}^{k} w_i}$$

where $w_i = -\log(K_{di})$ reflects binding potency at target $t_i$ and $\bar{F}_{T(t_i)}$ is the mean female fraction for that target class. This linear model provides a first-approximation prediction for preclinical safety planning.

### Comparison with Pharmacogenomic Sex Differences

Our findings complement and extend the pharmacogenomic literature on sex differences in drug response. Soldin and Mattison [10] documented sex-differential activity of CYP1A2 (higher in males), CYP2D6 (complex, substrate-dependent), CYP3A4 (higher in females), and UGT enzymes (generally higher in males). These pharmacokinetic sex differences create a baseline signal affecting all drugs metabolized by these enzymes, regardless of target.

Our molecular sex axis represents a pharmacodynamic layer operating independently of these pharmacokinetic differences. Drugs sharing metabolic pathways (e.g., CYP3A4-metabolized drugs) show dramatically different positions on the axis---from 31.4%F (androgen receptor targets) to 90.5%F (estrogen receptor targets)---demonstrating that target-mediated pharmacodynamic sex differences can override the common pharmacokinetic background.

Mauvais-Jarvis et al. [22] emphasized that sex differences operate at every level: genetic (X-chromosome dosage), epigenetic (X-inactivation), hormonal, and cellular. Our molecular sex axis can be understood as the pharmacological expression of these multi-level sex differences, integrated through the drug target as a molecular nexus. Anderson [23] demonstrated that even after correcting for body weight, lean body mass, and hepatic blood flow, significant residual sex differences persist in drug pharmacodynamics. Our data provide a mechanistic explanation: the drug target itself is a sex-differential molecular entity producing sex-differential downstream effects.
...
**Immune targets** (TNF-alpha 69.1%F, COX 69.8%F, CGRP 81.4%F): The female immune hypersensitivity---driven by X-linked immune genes (including TLR7, TLR8, FOXP3), estrogen-enhanced antibody production, and higher CD4+ T-cell counts [4,6]---creates a pharmacological vulnerability space where immune-targeting drugs disproportionately affect women. Biallelic expression of X-linked immune genes in some female cells (due to incomplete X-inactivation) creates baseline immune hyperactivation that amplifies drug-induced immune modulation.

The CGRP pathway result (81.4%F) deserves special attention. The strong female bias of CGRP-targeting drugs (erenumab, fremanezumab, galcanezumab, rimegepant) reflects both the 3:1 female predominance of migraine and the sex-differential biology of the trigeminal vascular system. Estrogen modulates CGRP release from trigeminal neurons, and menstrual cycle fluctuations trigger CGRP-mediated migraine attacks [24]. This creates a pharmacological feedback loop: CGRP drugs are prescribed predominantly to women, but also produce genuinely sex-differential pharmacodynamic effects through estrogen-CGRP crosstalk.

**Hormonal targets** (androgen receptor 31.4%F, GnRH agonists ~35%F): The Reproductive Paradox applies: drugs targeting sex hormone receptors produce opposite-sex-biased safety signals because minority-sex users show disproportionate susceptibility. Androgen receptor antagonists (enzalutamide, abiraterone, bicalutamide), while prescribed predominantly to men, produce signals that reveal women prescribed these drugs experience disproportionately high rates of certain AEs.
...
The drug class convergence data provide additional evidence against confounding. If the statin sex signature (52.0%F, 3.4 pp range) were driven by prescribing patterns, the within-class range would be wider. The tight convergence instead implicates HMG-CoA reductase as the determinant of sex-differential safety, regardless of which statin is used.

### Clinical Utility

1. **Preclinical safety prediction:** Target-based sex prediction should be integrated into toxicology planning. For immune targets, female-enriched animal models should be used. Drugs targeting the female extreme (>65%F) should include higher proportions of female animals and sex-stratified endpoints.

2. **Clinical trial design:** Sex-stratified analysis should be mandatory for drugs targeting the axis extremes. Power calculations should account for the expected effect size from the target class position on the molecular sex axis.

3. **Post-market surveillance:** New drugs entering well-characterized target spaces (e.g., a new CGRP antagonist) can inherit the sex-safety predictions of the target class. Regulatory agencies could implement targeted sex-stratified signal detection for high-risk target classes.

4. **Drug labeling:** For drugs at the axis extremes, product labels should include sex-specific AE incidence rates, dose recommendations where data support them, and sex-specific monitoring guidance.
...
5. FAERS data are subject to reporting bias, notoriety bias, and the Weber effect. While sex-stratified disproportionality analysis mitigates some biases, residual confounding cannot be excluded.

6. The analysis does not incorporate preclinical data on sex-differential target expression or downstream signaling. Integration of GTEx expression data, sex-stratified GWAS, and in vitro pharmacology could improve mechanistic understanding.
...
**Figure 5.** Protein family distribution across the molecular sex axis. Violin plots showing drug female fractions within each major protein family. Nuclear receptors show bimodal distribution (anchoring both extremes), while kinases cluster near the neutral midpoint.

**Figure 6.** Drug pair divergence network. Top 50 most divergent drug pairs (sharing >= 20 AEs), with edge weight proportional to divergence. Immune-targeting drugs form a female-biased cluster; hormonal and neurological drugs form a male-biased cluster.