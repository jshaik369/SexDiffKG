...
3. **Validation limited to 15 benchmarks.** The initial validation set, while drawing from landmark publications, is small relative to the 183,539 signals generated. Expanded validation against comprehensive resources (e.g., the Zurich Sex-Specific Side Effect Resource, PharmGKB annotations) would strengthen confidence.

4. **Embedding performance is baseline.** MRR 0.039 is modest and reflects minimal hyperparameter tuning (10 epochs, single model architecture). Systematic comparison of TransE, DistMult, ComplEx, and RotatE with proper hyperparameter search would establish more competitive baselines.
...
SexDiffKG is the first knowledge graph to structurally encode biological sex on every drug-safety edge. Built from 87 FAERS quarterly releases (14.5M deduplicated reports) with three-tier drug normalization and multi-source molecular integration, it identifies 183,539 sex-differential signals across 3,441 drugs and 5,658 adverse events. Validation against literature benchmarks achieves 86.7% coverage and 61.5% directional precision. Baseline DistMult embeddings (MRR 0.039) provide initial link prediction capability. SexDiffKG offers a computational foundation for sex-aware pharmacovigilance.
...
7. Lucena MI et al. Susceptibility to amoxicillin-clavulanate-induced liver injury. Hepatology. 2009;49(6):2001-9. PMID:25475693
...
- **Issue:** 15 benchmarks is insufficient for 183,539 signals
...
### W3. HIGH — DistMult MRR is low (0.039)
- **Issue:** MRR 0.039 means average correct entity is ranked ~26th
...
| "15 benchmarks is too few" | HIGH | HIGH | Expand to 40 (W2) |
...
| 7 | AMOXICILLIN | Hepatocellular injury | M>F | Lucena MI. Hepatology 2009 | 25475693 | ✅ CORRECT |
...
**Version:** 2.0 (post-normalization, post-DistMult v2)
**Infrastructure:** NVIDIA DGX Spark GB10 (ARM64, 120GB unified)
...
| 9. DistMult v2 | ✅ Complete | MRR 0.039, Hits@10 6.9% |
...
| 13:01 | DistMult v2 COMPLETE | MRR 0.039, Hits@10 6.9% |
