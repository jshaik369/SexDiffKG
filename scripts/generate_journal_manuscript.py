...
S.append(Paragraph(
    ...
    "(200 dimensions, 100 epochs) achieved MRR of 0.048, Hits@10 of 8.85%, and AMRI of 0.981, "
    ...
    "literature-documented sex-differential drug safety benchmarks achieved 67% inclusive confirmation "
    "rate (9/15 drugs found, 6/9 confirmed or weakly confirmed). SexDiffKG is, to our knowledge, the "
...
S.append(Paragraph(
    ...
    "(3) embeds the full graph using DistMult to enable sex-aware link prediction, with AMRI of 0.981; "
    "(4) reveals 429 gene targets with measurable sex-differential drug safety profiles; (5) validates "
    "findings against 15 literature benchmarks, achieving 67% confirmation rate; and (6) provides a "
...
S.append(Paragraph(
    ...
    "CPU training due to NVRTC JIT compilation limitations on the GB10 for complex-valued operations.",
    body))
...
S.extend(tbl(
    ...
    [['Metric', 'DistMult', 'RotatE', 'Interpretation'],
     ...
     ['Hits@3', '4.54%', '0.002%', '—'],
     ['Hits@10', '8.85%', '0.009%', '—'],
     ['AMRI', '0.981', '0.003', 'Top 1.9% vs near-random'],
     ['AMR', '~1,206', '62,350', 'Out of 126,575 entities']],
    [1.0, 0.8, 0.8, 3.2]))

S.append(Paragraph(
    "The AMRI of 0.981 is the most informative metric: it indicates the model consistently ranks correct "
...
S.extend(tbl(
    "<b>Table 9.</b> Validation against 15 literature-documented benchmarks.",
...
S.append(Paragraph(
    "Hit rate: 9/15 drugs found in KG (60%), of which 6/9 confirmed or weakly confirmed (67% inclusive). "
    "The 67% confirmation rate is reasonable for spontaneous reporting data validation, given FAERS drug "
...
S.append(Paragraph(
    "All computation was performed on a single NVIDIA DGX Spark (Grace Blackwell GB10, ARM64, 128GB "
...
S.append(Paragraph(
    ...
    "infrastructure was provided by an NVIDIA DGX Spark (Grace Blackwell GB10). The author thanks the FDA "
