...
S.append(Paragraph("ORCID: 0009-0003-2065-4673", orcid_style))
S.append(Spacer(1, 16))
...
S.append(Paragraph(
    ...
    "Knowledge graph embedding using DistMult achieved MRR of 0.048, Hits@10 of 8.85%, and AMRI of 0.981. "
    ...
    "sex-biased drug safety patterns. Signal validation achieved 67% confirmation rate. <b>Conclusions:</b> SexDiffKG is "
...
S.append(Paragraph(
    ...
    "(5) validate findings against 15 literature benchmarks; and (6) provide a reproducible, publicly-deposited resource "
...
S.extend(tbl(
    ...
    [['Parameter', 'DistMult', 'RotatE'],
     ...
     ['Loss', 'SLCWA', 'SLCWA'],
     ['Hardware', 'GPU (GB10)', 'CPU (Grace Blackwell)'],
...
S.append(Paragraph(
    ...
    "<b>Validation:</b> Tested against 15 literature-documented benchmarks. "
...
S.extend(tbl(
    ...
    [['Metric', 'DistMult', 'RotatE', 'Interpretation'],
     ...
     ['AMRI', '0.981', '0.003', 'Top 1.9% of candidates'],
     ['AMR', '~1,206', '~62,350', 'DistMult vs near-random']],
    [1.0, 0.8, 0.8, 3.4]))

S.append(Paragraph(
    "AMRI 0.981 indicates DistMult consistently ranks correct triples near top despite 126K entities and imbalanced "
...
S.extend(tbl(
    "<b>Table 9.</b> Validation against 15 literature benchmarks.",
    [['Outcome', 'Count', 'Status'],
     ...
     ['Hit Rate (Drug Found)', '9/15', '60.0% (4,455/29,277 drugs mapped to ChEMBL)'],
     ['Confirmation (Found)', '6/9', '66.7% inclusive (confirmed + weakly confirmed)']],
    [1.2, 0.6, 4.3]))

S.append(Paragraph(
    "67% confirmation rate reasonable given FAERS name normalization, MedDRA term differences, and real-world vs trial "
...
S.append(Paragraph(
    ...
    "and linking to 429 gene targets with sex-biased patterns provides new insights. Validation (67% confirmation) and audit "
...
S.append(Paragraph(
    "All computation on NVIDIA DGX Spark (Grace Blackwell GB10, ARM64, 128GB). Complete dataset deposited on Zenodo "
    ...
    "Runtime on GB10: ~15h wall-clock (data 2h, ROR 1h, DistMult 3.5h, RotatE 7.5h, post-processing 1h).",
    body_noindent))
...
S.append(Paragraph(
    "Independent research at CoEvolve Network, Barcelona. Computational infrastructure (NVIDIA DGX Spark, GB10) via "
